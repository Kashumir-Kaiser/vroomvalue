...
from sqlalchemy import Date, DateTime, Float, Integer, String, create_engine, func, select
...
def save_feedback(record: FeedbackRecord) -> None:
    with Session(engine()) as session:
        ...
        existing = session.scalar(
            select(FeedbackRecord).where(FeedbackRecord.prediction_id == record.prediction_id)
        )
        if existing is not None:
            raise ValueError("Feedback already exists for this prediction id.")
        session.add(record)
        session.commit()
